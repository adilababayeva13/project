...
export const jobLoadAction = (
  ...
  try {
    const { data } = await axios.get(
      `http://localhost:9000/api/jobs/show/?pageNumber=${pageNumber}&keyword=${keyword}&cat=${cat}&location=${location}`,
    )
    dispatch({
...
export const deleteSingleJobAction = (job_id) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.delete(
      `http://localhost:9000/api/job/delete/${job_id}`,
    )
    dispatch({
...
export const registerAjobAction = (job) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post(
      'http://localhost:9000/api/job/create',
      job,
    )
    dispatch({
